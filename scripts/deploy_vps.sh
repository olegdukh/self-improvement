...
RUNNER_TOKEN=$(curl -X POST -H "Authorization: token $GH_PAT" \
  ...
  "[https://api.github.com/repos/$REPO_OWNER/$REPO_NAME/actions/runners/registration-token](https://api.github.com/repos/$REPO_OWNER/$REPO_NAME/actions/runners/registration-token)" \
  | jq -r '.token')
...
su - runner -c "./config.sh --url [https://github.com/$REPO_OWNER/$REPO_NAME](https://github.com/$REPO_OWNER/$REPO_NAME) --token $RUNNER_TOKEN --name $SERVER_NAME --ephemeral --unattended"
...
SERVER_ID=\$(curl -s [http://169.254.169.254/v1/meta-data/instance-id](http://169.254.169.254/v1/meta-data/instance-id))
curl -X DELETE -H "Authorization: Bearer $HCLOUD_TOKEN" "[https://api.hetzner.cloud/v1/servers/](https://api.hetzner.cloud/v1/servers/)\$SERVER_ID"
EOF
